...
knitr::opts_chunk$set(warning = F, message = F, dpi = 600, fig.align = 'center')
```

# Introduction to the framework and application
***
<br>
In this notebook, we run through the basic steps to implement the geospatial data integration framework (G-DIF) presented in "A geospatial data integration framework to rapidly estimate post-disaster damage" by Sabine Loos, Jack Baker, David Lallemant, and **sabine add other authors**.  

The **goal** of this paper is to provide a framework to integrate multiple damage data sources that become available after an earthquake, including: field surveys, remote-sensing proxies, engineering forecasts, and other geospatial covariates. More information can be found in the paper, but here is the general flowchart:

```{r flowchart, out.width="70%"}
...
The code for this is split between this main markdown and multiple functions. The entire source code can be found at [https://github.com/sabineloos/GDIF-damageprediction](https://github.com/sabineloos/GDIF-damageprediction).
...
3. A self-developed **engineering forecast** of damage. This uses the USGS shaking intensity + fragility curves from NSET + Landscan population exposure to predict the mean damage ratio at a 1km resolution [@Jarvis2008, @LandScan:2012tj]
4. A [**damage proxy map (DPM)**](https://aria-share.jpl.nasa.gov/) developed through inSAR-based coherence differencing from the ARIA project at the NASA Jet Propulsion Lab. This provides a damage proxy number from -1 to 1 at 30m resolution [@Yun2015]
...
alldat_shp <- readRDS(file = "data/Nepal_11dist_damagedata_grids.rds")
alldat_shp@data[1:5,]
```
...
- `nmin` and `nmax` = Fpr the spatial correlation model, the minimum and maximum number of points to consider as an alternative to `maxidst.multiplier`
...
leg <- ggplot(comb_wide) + geom_histogram(aes(GT_OUT, fill = locns)) +
  scale_fill_manual("", values = cols)+theme(legend.direction = "vertical", legend.spacing = unit(0.05, "cm"),text = element_text(colour = "grey30", family = "Open Sans"))
...
p.corr <- ggpairs(dplyr::filter(comb_wide, ENG_mnDR > 0),columns = 1:5,
  ...
  theme_minimal()+theme(panel.grid = element_blank(),text = element_text(colour = "grey30", family = "Open Sans"),
...
p.coeff <- plot_coeff(var_vec = names(RK_trend$se$ols.coeff),
           coeff_vec = RK_trend$se$ols.coeff,
           se_vec = RK_trend$se$ols,
           names_vec = c('Intercept','Eng. forecast',"Damage proxy map", 'Shaking intensity','Elevation'),
           var_color = GDIF_col)
p.coeff
```
...
# plot beta and standard error for trend model with DPM
plot_coeff(var_vec = names(RK_trend_nodpm$se$ols.coeff),
           coeff_vec = RK_trend_nodpm$se$ols.coeff,
           se_vec = RK_trend_nodpm$se$ols,
           names_vec = c('Intercept', 'Shaking intensity','Eng. forecast','Elevation'),
           var_color = GDIF_col)
```
...
c0 = resid.vgm$var_model$psill[which(RK_trend$vgm$var_model$model %in% "Nug")]
c0_field = orig.vgm$var_model$psill[which(RK_trend$vgm$var_model$model %in% "Nug")]

# c1 = sum(resid.vgm$var_model$psill)
# h = seq(0, max(resid.vgm$exp_var$dist), by = 1)
# k = resid.vgm$var_model$kappa[which(resid.vgm$var_model$model %in% "Exp")]
r =resid.vgm$var_model$range[which(resid.vgm$var_model$model %in% "Mat")]

p.vario <- plot_vario(variomod_trend = orig.vgm, 
                      variomod_detrend = resid.vgm, 
                      c0_trend = c0_field, c0_detrend = c0, r_detrend = r,
...
pred_shp$RK_pred_fin <- pred_shp$RK_pred_fin
...
ind_field <- which(pred_shp$GRID_ID %in% field_sample_shp$GRID_ID)
...
```{r plotting single scenario error, out.width='80%'}
...
p.error <- ggplot(dplyr::filter(error_df_melt, Model %in% c("RK", "ENG"))) +
  ...
  geom_text(data = ME[1:2,], aes(x = value, y = ME$yend, color = Model), family = "Open Sans",
            label = c(paste("Mean =", round(ME$value[1],3)),
              paste("Mean =", round(ME$value[2],3))),
            hjust = "right", nudge_x = -0.01, size = 3, nudge_y = -0.001)+
  geom_text(data = VE[1:2,], aes(x = x, y = y, color = Model),family = "Open Sans",
            label = c(paste("Standard dev. =", round(VE$value[1],3)),
              paste("Standard dev. =", round(VE$value[2],3))),
            hjust = "left", vjust = "bottom",size = 3, nudge_y = 0.001, nudge_x = 0.01)+
...
```{r plotting results from UQ, fig.align='center', echo=F, results='hide', out.width='100%', fig.asp=1.5, eval=F}
# output from RK on sherlock
GDIF_runs <- read.csv("data/GDIF_runs.csv")
head(GDIF_runs)
GDIF_runs$model <- "RK"
# plot the mean squared error
p.MSE_SA <- plot_SA_ridgeline(SA_data = GDIF_runs, xvar_char = "MSE", yvar_char = "nsurvey", error_eng = errormet_ENG$errormet$MSE,
                  colvar_char = "model", cols = cols_gdifeng, cols_labs = labs, 
                  x_title = "Mean Squared Error", y_title = expression("Number of field surveyed locations, "~ n[fs]), x_lims = c(0.5, 2.25))
# plot the mean error
p.ME_SA <- plot_SA_ridgeline(SA_data = GDIF_runs, xvar_char = "ME", yvar_char = "nsurvey", error_eng = errormet_ENG$errormet$ME,
                  colvar_char = "model", cols = cols_gdifeng, cols_labs = labs, 
                  x_title = "Bias (Mean Error)", y_title = expression("Number of field surveyed locations, "~ n[fs]), x_lims = c(-1,1.01))+
  geom_vline(mapping = aes(xintercept = 0),size = .2, lty = "dotted", color = "black") + theme(legend.position = c(0.8,0.8))
...
ggsave(plot = p.corr,"results/Images/correlationmat_seed977.pdf", width = 6.5, height = 6.5, units = "in",dpi = 600)

## COEFFICIENTS AND VARIOGRAM (figure 5)
# coefficients
saveplot(plot = p.coeff,width = 3.25, height = 3, file_locn = "results/Images/", 
         file_name = paste0("OLStrend_singlescenario_seed977"))
# variogram
saveplot(plot = p.vario,width = 3.25, height = 3, file_locn = "results/Images/", 
         file_name = paste0("MaternVario_singlescenario_seed977"))
...
saveplot(plot = p.rkpredfin,width = 3.25, height = 2.6, file_locn = "results/Images/", 
         file_name = paste0("map_RKpredfin_11dist_singlescenario_seed977"))
# final variance
saveplot(plot = p.rkvarfin,width = 3.25, height = 2.6, file_locn = "results/Images/", 
         file_name = paste0("map_RK_varfin_11dist_singlescenario_seed977"))
# true damage (full map)
saveplot(plot = p.GT,width = 3.25, height = 2.6, file_locn = "results/Images/", 
         file_name = paste0("map_GT_mnDG_11dist"))
# engineering values in mean damage grade
saveplot(plot = p.ENGval,width = 3.25, height = 2.6, file_locn = "results/Images/", 
         file_name = paste0("map_ENGVAL_11dist_singlescenario_seed977"))

##ERROR HISTOGRAM (figure 7)
saveplot(plot = p.error,width = 6.5, height = 3, file_locn = "results/Images/", 
         file_name = paste0("hist_error_11dist_singlescenario_seed977"))
ggsave(plot = p.error,filename =  "results/Images/hist_error_11dist_singlescenario_seed977.pdf", width = 6.5, height = 3, units = "in",dpi = 600)
...
# ggsave(plot = p.MSE_SA,filename =  "results/Images/MSE_SA.pdf", width = 3.25, height = 2.6, units = "in",dpi = 600)
# # mean error
# saveplot(plot = p.ME_SA, width = 3.25, height = 2.6, file_locn = "results/Images/",file_name = "ME_SA")
# ggsave(plot = p.ME_SA,filename =  "results/Images/ME_SA.pdf", width = 3.25, height = 2.6, units = "in",dpi = 600)
```

```{r for saving, echo=F, results='hide'}
# merge predictions and errors with grid shape (some grids of grid shape willl have no predictions)
alldat_shp <- merge(alldat_shp, pred_shp@data[,c(id_var, "RK_pred_fin", "RK_var_fin", "ERROR", "ENG_ERROR")], by = id_var, all.x = T)
# save
```
